      ...
      _.each(models,function eachInstantiatedModel(thisModel, modelID) {
        ...
        if (sails.config.globals && sails.config.globals.models) {
          ...
          if (improved) {
            ...
            globalName.reduce(function (context, part, index) {
              if (index === globalName.length - 1) {
                context[part] = thisModel;
                if (sails.config.globals.models.traditional) {
                  global[part] = thisModel;
                }
              } else {
