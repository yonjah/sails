module.exports = function(sails) {
  ...
  return {
    ...
    loadModels: function (cb) {
      ...
      var improved = false;
      if (sails.config.globals && sails.config.globals.models === 'improved') {
        improved = true;
      }
      if (!improved) {
        console.log(sails.config.models);
        sails.log.warn('`sails.config.globals.models` traditional will be deprecated');
      }

      buildDictionary.optional({
        dirname   : sails.config.paths.models,
        filter    : /^([^.]+)\.(js|coffee|litcoffee)$/,
        replaceExpr : improved ? null : /^.*\//,
        keepDirectoryPath: improved,
        flattenDirectories: true
      }, function(err, models) {
        if (err) {return cb(err);}
      ...
      async.auto({
        ...
        _.extend(hooks, _.reduce(results.nodeModulesFolder, function(memo, module, identity) {
          ...
          if (module['package.json'] && module['package.json'].sails && module['package.json'].sails.isHook) {
            ...
            // If an identity was specified in sails.config.hooks, use that
            if (sails.config.hooks && sails.config.hooks[identity] && sails.config.hooks[identity].identity) {
              hookName = sails.config.hooks[identity].identity;
            }
            ...
            // If we have a core hook or a hook in api/hooks with this name, throw an error
            if (sails.hooks[hookName] || hooks[hookName]) {
              cb(new Error("Tried to load hook `" + hookName + "`, but a hook with that name already exists."));
            }

            // Add this to the list of hooks to load
            memo[hookName] = require(path.resolve(sails.config.appPath, "node_modules", identity));
          }
          return memo;
        }, {}));
